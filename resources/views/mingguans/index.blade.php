...
<div class="card">
    <div class="card-header">
        <div class="d-flex justify-content-between align-items-center">
            <h4>Media Cetak Mingguan</h4>
            <div class="d-flex">
                <form class="d-flex" action="{{ route('mingguans.search') }}" method="GET">
                    @csrf
        ...
        <a href="{{ route('mingguans.create') }}" class="btn btn-primary">Tambah Data</a>
    </div>
    <!-- /.card-body -->
    <div class="card-body">
                        ...
                        <tr>
                            ...
                            <td class="text-center">
                                @if ($mingguan->lampiran)
                                    <a href="{{ asset('storage/' . $mingguan->lampiran) }}">Download Lampiran</a>
                                @else
                                    Tidak Ada Lampiran
                                @endif
                                <div class="action-buttons">
                                    <a href="{{ route('mingguans.edit', $mingguan->id) }}" class="btn btn-success btn-sm">Edit</a>
                                    <form action="{{ route('mingguans.destroy', $mingguan->id) }}" method="POST" onsubmit="return confirm('Apakah kamu yakin ingin menghapus data ini?')">
                                        @method('DELETE')
                                        @csrf
