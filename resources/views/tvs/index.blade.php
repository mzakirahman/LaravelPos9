...
<div class="card">
    <div class="card-header">
        <div class="d-flex justify-content-between align-items-center">
            <h4>Media Tv</h4>
            <div class="d-flex">
                <form class="d-flex" action="{{ route('tvs.search') }}" method="GET">
                    @csrf
        ...
        <a href="{{ route('tvs.create') }}" class="btn btn-primary">Tambah Data</a>
    </div>
    <!-- /.card-body -->
    <div class="card-body">
                ...
                <tbody>
                    @foreach ($tvs as $tv)
                        <tr>
                            ...
                            <td class="text-center">{{ $tv->bulan }}</td>
                            <td class="text-center">
                                @if ($tv->lampiran)
                                    <a href="{{ asset('storage/' . $tv->lampiran) }}">Download Lampiran</a>
                                @else
                                    Tidak Ada Lampiran
                                @endif
                                <div class="action-buttons">
                                    <a href="{{ route('tvs.edit', $tv->id) }}" class="btn btn-success btn-sm">Edit</a>
                                    <form action="{{ route('tvs.destroy', $tv->id) }}" method="POST" onsubmit="return confirm('Apakah kamu yakin ingin menghapus data ini?')">
                                        @method('DELETE')
                                        @csrf
